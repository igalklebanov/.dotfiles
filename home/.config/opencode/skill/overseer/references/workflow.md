...
**VCS Required:** Creates bookmark `task/<id>`, records start commit. Fails with `NotARepository` if no git found.
...
**VCS Required:** Commits changes (NothingToCommit treated as success), then deletes the task's bookmark (best-effort) and clears the DB bookmark field on success. Fails with `NotARepository` if no git found.
